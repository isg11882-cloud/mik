...
async function callOllamaJSON(prompt) {
  const res = await fetch(`${OLLAMA_URL}/api/generate`, {
    ...
    body: JSON.stringify({
      ...
      stream: false,
      format: 'json',
      // 한국어는 1글자 = 2~3토큰이므로 2000토큰으로 여유 확보
  ...
  if (fb === -1) throw new Error(`No JSON in response. Got: ${text.slice(0, 100)}`);

  let lb = cleaned.lastIndexOf('}');

  // JSON이 잘린 경우(lb === -1 또는 lb <= fb) 자동 복구 시도
  if (lb === -1 || lb <= fb) {
    // 닫히지 않은 문자열·배열·객체를 닫아서 파싱 재시도
    let fragment = cleaned.slice(fb);
    // 열린 배열 닫기
    ...
    try {
      ...
      throw new Error(`JSON truncated and repair failed. Got: ${text.slice(0, 120)}`);
    }
  }
...
// ─────────────────────────────────────────────
// ─────────────────────────────────────────────
// 진단 모드: node run_local_ai.js --diagnose
// ─────────────────────────────────────────────
async function runDiagnose() {
  ...
  try {
    ...
    const d = await r.json();
    const models = (d.models || []).map(m => m.name);
    console.log(`  ✅ Ollama 정상 | 모델: ${models.join(', ') || '없음'}`);
  } catch (e) {
    console.log(`  ❌ Ollama 연결 실패: ${e.message}`);
    console.log(`  → Ollama가 실행 중인지 확인: http://localhost:11434`);
  }

  // 2. Worker API /api/pending
  console.log('② Worker API /api/pending 확인...');
  try {
    const r = await fetch(`${WORKER_URL}/api/pending?limit=3`, { signal: AbortSignal.timeout(10000) });
    ...
    if (r.ok) {
      ...
      console.log(`  ❌ 오류 응답: ${text.slice(0, 200)}`);
    }
  } catch (e) {
    console.log(`  ❌ Worker API 접근 실패: ${e.message}`);
    console.log(`  → Worker URL: ${WORKER_URL}`);
  }

  // 3. Worker API /api/articles
  console.log('③ Worker API /api/articles 확인...');
  try {
    const r = await fetch(`${WORKER_URL}/api/articles?limit=3`, { signal: AbortSignal.timeout(10000) });
    const text = await r.text();
    console.log(`  HTTP ${r.status}`);
    if (r.ok) {
      const d = JSON.parse(text);
      const arts = d.articles || d;
      console.log(`  ✅ 기사 총 ${d.total}건, 반환: ${arts.length}건`);
      if (arts.length > 0)
  ...
  // 4. Ollama 간단 JSON 생성 테스트
  console.log('④ Ollama JSON 생성 테스트...');
  try {
    const result = await callOllamaJSON(`Output exactly this JSON: {"test":"ok","value":1}`);
    console.log(`  ✅ JSON 생성 정상:`, JSON.stringify(result));
  } catch (e) {
